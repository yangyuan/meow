...
VOID MeowWindowManager::SetCompositionText(TCHAR * text) {
	if (impl_composition != NULL) {
	...
	case WM_MOUSEMOVE:
		if (wParam == MK_LBUTTON) {
			...
			}
			{
				...
				rect.left = me->pointnow.x - me->pointcaptureclient.x; // x
				rect.top = me->pointnow.y - me->pointcaptureclient.y; // y
...
VOID MeowCompositionWindow::WndInit(HWND hwnd, MeowWindow * me) {
...
VOID MeowCompositionWindow::SetText(TCHAR * w) {
	if (hwnd_edit != NULL) {
